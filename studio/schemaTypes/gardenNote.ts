...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'number',
      description: 'Epistemic confidence from 1 to 100.',
  ...
  preview: {
    ...
    prepare({title, status, tended}) {
      const date = tended ? new Date(tended).toLocaleDateString() : 'Untended'
      const readableStatus = status ? status[0].toUpperCase() + status.slice(1) : 'Unknown'

      return {
        title,
        subtitle: `${readableStatus} · ${date}`,
      }
    },
  },
})
